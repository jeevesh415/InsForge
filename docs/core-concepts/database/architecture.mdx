...
| **ai** | Deprecated AI configuration & usage tables from older deployments | Internal migrations only |
...
#### `ai` Schema (Deprecated AI Proxy)

| Table | Purpose |
|-------|---------|
| `configs` | Deprecated model configuration table; dropped by current migrations when present |
| `usage` | Deprecated token usage table; dropped by current migrations when present |
...
InsForge supports developer-facing custom migrations through the admin API and the InsForge CLI. For the day-to-day workflow, use the [Database Migrations](/core-concepts/database/migrations) guide.
...
- The executor sets `search_path` to `public`, so unqualified app tables resolve there by default
