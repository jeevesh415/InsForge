...
### Disabling New User Signups

When a project is in closed-beta, at capacity, or under a bot wave, you can block all new account creation without taking auth down.

**Dashboard:** Auth Settings → General → **Disable New User Signups** toggle.
...
Behavior when the toggle is **on**:

| Scenario | Result |
|----------|--------|
| `POST /api/auth/users` without admin token | `403 AUTH_SIGNUP_DISABLED` |
| First-time OAuth login (unknown account) | `403 AUTH_SIGNUP_DISABLED` |
| Existing user signs in (email or OAuth) | Allowed — unaffected |
| Admin creates user via dashboard or API | Allowed — bypass is automatic when bearer token carries `role: project_admin` |
...
<CardGroup cols={2}>
  ...
  <Card title="Signup Toggle" icon="toggle-on">
    Disable new user signups instantly while existing users keep signing in
  </Card>
</CardGroup>
